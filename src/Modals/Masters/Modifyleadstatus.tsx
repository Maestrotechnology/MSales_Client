...
export default function Modifyleadstatus({
  type,
  Editdata,
  close,
  handleMasterList,
  ...
  const { values, setValues, handleSubmit, setFieldValue, errors, touched } =
    useFormik({
      ...
      onSubmit: (values) => {
        if (type === "add") {
          handleLeadstatus(values);
        } else if (type === "update") {
          handleUpateleadstatus(values);
        }
      },
    });

  // Add master
  const handleLeadstatus = (values: any) => {
    let finalObj = { ...values, token: token };
    createleadStatus(JSONtoformdata(finalObj))
      .then((res) => {
  ...
  // Update Master
  const handleUpateleadstatus = (values: any) => {
    let finalObj = {
      ...values,
      token: token,
      dataId: Editdata?.LeadStatusId,
    };
    updateleadStatus(JSONtoformdata(finalObj))
      .then((res) => {
        if (res.data.status === 1) {
  ...
  const handleSetValues = (data: any) => {
    setValues({ ...values, name: data?.LeadStatusName });
  };
    ...
    <>
      ...
      <div className={classes.btnactionblock}>
        <CommonButton
          ...
          color="#004c97"
          Disabled={Editdata?.LeadStatusName === values.name ? true : false}
