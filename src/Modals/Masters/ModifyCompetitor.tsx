...
export default function ModifyCompetitor({
  type,
  Editdata,
  close,
  handleMasterList,
  ...
  const { values, setValues, handleSubmit, setFieldValue, errors, touched } =
    useFormik({
      ...
      onSubmit: (values) => {
        if (type === "add") {
          handleAddCompetitor(values);
        } else if (type === "update") {
          handleUpateCompetotor(values);
        }
      },
    });

  // Add master
  const handleAddCompetitor = (values: any) => {
    let finalObj = { ...values, token: token };
    createcompetitor(JSONtoformdata(finalObj))
      .then((res) => {
  ...
  // Update Master
  const handleUpateCompetotor = (values: any) => {
    let finalObj = {
      ...values,
      token: token,
      dataId: Editdata?.CompetitorsId,
    };
    updatecompetitor(JSONtoformdata(finalObj))
      .then((res) => {
        if (res.data.status === 1) {
  ...
  const handleSetValues = (data: any) => {
    setValues({ ...values, name: data?.CompetitorsName });
  };
    ...
    <>
      ...
      <div className={classes.btnactionblock}>
        <CommonButton
          ...
          color="#004c97"
          Disabled={Editdata?.CompetitorsName === values.name ? true : false}
