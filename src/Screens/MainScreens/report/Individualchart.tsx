...
import IndividualPerformanceChart from "../../../Components/Chart/IndividualPerformance";
...
export default function IndividualChart() {
  ...
  const {
    ...
    onSubmit: () => {
      ...
      handleReportData(1, performancelist.size, values);
    },
  });
  const handleReportData = (page: number, size: number, values: any) => {
    ...
    performanceReport(page, size, formData)
      .then((res) => {
  ...
  useEffect(() => {
    if (token) {
      handleReportData(performancelist.page, performancelist.size, values);
  ...
  return (
    <>
      <PageHeader
        heading={`Individual Report (${getDates(
          values?.from_date,
          values?.to_date
        )})`}
