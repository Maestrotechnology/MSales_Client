...
import Modifyleadstatus from "../../../Modals/Masters/Modifyleadstatus";
...
export default function Leadstatus() {
  ...
  const handleMasterList = (page: number, size: number, values: any) => {
    ...
    ListleadStatus(page, size, JSONtoformdata(finalObj))
      .then((res) => {
  ...
  const handleDeleteMaster = () => {
    ...
    deleteleadStatus(JSONtoformdata(finalObj))
      .then((res) => {
        if (res.data.status === 1) {
  ...
  return (
    <div>
      ...
      <GlobalModal
        size={500}
        title={modifymodalData.data ? "Edit Lead Status" : "Add Lead Status"}
        OnClose={() => {
      ...
      >
        <Deleteconfirmation
          msg={"Are You Sure Delete This Lead Status?"}
          close={() => {
            setIsShowDeleteModal({
              status: false,
              data: null,
