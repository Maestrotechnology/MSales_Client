...
import ModifyCompetitor from "../../../Modals/Masters/ModifyCompetitor";
...
export default function Competitors() {
  ...
  const handleMasterList = (page: number, size: number, values: any) => {
    ...
    Listcompetitor(page, size, JSONtoformdata(finalObj))
      .then((res) => {
  ...
  const columns = [
    ...
    {
      title: "Competitors Name",
      dataIndex: "CompetitorsName",
  ...
  const handleDeleteMaster = () => {
    ...
    deletecompetitor(JSONtoformdata(finalObj))
      .then((res) => {
        if (res.data.status === 1) {
  ...
  return (
    <div>
      ...
      <GlobalModal
        size={500}
        title={modifymodalData.data ? "Edit Competitor" : "Add Competitor"}
        OnClose={() => {
      ...
      >
        <Deleteconfirmation
          msg={"Are You Sure Delete This Competitor?"}
          close={() => {
            setIsShowDeleteModal({
              status: false,
              data: null,
